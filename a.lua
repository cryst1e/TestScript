...
repeat
	wait()
until game.Players.LocalPlayer.PlayerGui:FindFirstChild("MapUIContainer"):FindFirstChild("MapUI").BoothUI
repeat
	wait()
...
while wait(_G.boardUpdateInterval) do
	...
	if tonumber(plast) > last then
		...
		if _G.autoUpdateGoal then
			...
			event:FireServer(_G.text..plast .. " / ", "booth")
		end
		last = tonumber(plast)
	end
end
